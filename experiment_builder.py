...
class ExperimentBuilder(object):
    def __init__(self, args, data, model, device):
        ...
        shutil.copy(self.args.name_of_args_json_file, self.args.experiment_name)
        ...
        print(self.args.continue_from_epoch)
        #assert self.args.continue_from_epoch == -2
    ...
    def build_loss_summary_string(self, summary_losses):
        ...
        for key, value in zip(list(summary_losses.keys()), list(summary_losses.values())):
            if "loss" in key or "accuracy" in key or "c-index" in key:
                value = float(value)
    ...
    def train_iteration(self, train_sample, sample_idx, epoch_idx, total_losses, current_iter, pbar_train):
        ...
        """
        support_ligand, support_prot, target_ligand, target_protein, support_labels, target_labels = train_sample
        data_batch = (support_ligand, support_prot, target_ligand, target_protein, support_labels, target_labels)

        if sample_idx == 0:
            print("shape of data", support_ligand.shape, support_prot.shape,\
                 target_ligand.shape, target_protein.shape, support_labels.shape, target_labels.shape)
    ...
    def evaluation_iteration(self, val_sample, total_losses, pbar_val, phase):
        ...
        """
        global epoch_val_labels, epoch_val_preds

        support_ligand, support_prot, target_ligand, target_protein, support_labels, target_labels = val_sample
        data_batch = (
            support_ligand, support_prot, target_ligand, target_protein, support_labels, target_labels)

        losses, _, my_preds, my_labels = self.model.run_validation_iter(data_batch=data_batch)
        epoch_val_labels.extend(list(my_labels.squeeze()))
        epoch_val_preds.extend(list(my_preds.squeeze()))
    ...
    def test_evaluation_iteration(self, val_sample, total_losses, pbar_test, phase="test"):
        """
        Runs a validation iteration, updates the progress bar and returns the total and current epoch val losses.
        :param val_sample: A sample from the data provider
        :param total_losses: The current total losses dictionary to be updated.
        ...
        :return: The updated val_losses, total_losses
        """
        global epoch_test_labels, epoch_test_preds
        ...
        losses, _, my_preds, my_labels  = self.model.run_validation_iter(data_batch=data_batch)
        epoch_test_labels.extend(list(my_labels.squeeze()))
        epoch_test_preds.extend(list(my_preds.squeeze()))
        ...
        test_losses = self.build_summary_dict(total_losses=total_losses, phase=phase)
        ...
        return test_losses, total_losses
    ...
    def test_experiment(self, model_path):
       
        global epoch_test_preds, epoch_test_labels
        ...
        with tqdm.tqdm(total=int(len(self.data.dataset_test.main_df) / self.args.batch_size)) as pbar_test:
            for sample_idx, test_sample in enumerate(self.data.get_test_batches()):
                test_losses, total_losses = self.test_evaluation_iteration(val_sample=test_sample,
                                                total_losses=total_losses,
                                                pbar_test=pbar_test)

        test_losses["val_c_index"] = ci_index(epoch_test_preds, epoch_test_labels)
        return test_losses["test_loss_mean"], test_losses["test_loss_std"], test_losses["val_c_index"]
            ...
            while (self.state['current_iter'] < (self.args.total_epochs * self.args.total_iter_per_epoch)):
                ...
                for train_sample_idx, train_sample in enumerate(
                    ...
                    if self.state['current_iter'] % self.args.total_iter_per_epoch == 0:
                        ...
                        total_losses = dict()
                        val_losses = dict()
                        with tqdm.tqdm(total=int(len(self.data.dataset_val.main_df) / self.args.batch_size)) as pbar_val:
                            for _, val_sample in enumerate(self.data.get_val_batches()):
                                val_losses, total_losses = self.evaluation_iteration(val_sample=val_sample,
                                                                                     total_losses=total_losses,
                                                                                     pbar_val=pbar_val, phase='val')

                            val_losses["val_c_index"] = ci_index(epoch_val_preds, epoch_val_labels)

                            if val_losses["val_loss_mean"] < self.state['best_val_loss']:
                                print("Best validation loss", val_losses["val_loss_mean"])
                                self.state['best_val_loss'] = val_losses["val_loss_mean"]
                                self.state['best_c_index'] = val_losses["val_c_index"]
